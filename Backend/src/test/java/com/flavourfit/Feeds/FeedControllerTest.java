...
public class FeedControllerTest {
    ...
    public void removeCommentsByFeedIDTest() throws Exception {
        ...
        when(feedService.removeCommentFromFeed(commentID)).thenReturn(mockFeed);
        ...
        //Fail case
        when(feedService.removeCommentFromFeed(commentID)).thenThrow(new RuntimeException("Failed to remove the comment from feed"));
        mockMvc.perform(delete("/feeds/comment-feed")
                        .param("feedID", String.valueOf(feedID))
                        .param("commentID", String.valueOf(commentID)))
                .andExpect(status().isBadRequest())
    ...
    public void getAllFeedsByUserTest() throws Exception {
        int userId = 1;
        String offset = "0";
        ArrayList<FeedDto> mockFeeds = new ArrayList<>(); // Create a mock list of FeedDto objects
        when(authService.extractUserIdFromToken(anyString())).thenReturn(userId);
        when(feedService.getFeedsByUser(userId, Integer.parseInt(offset))).thenReturn(mockFeeds);

        //Invalid token
        when(authService.extractUserIdFromToken(anyString())).thenThrow(new RuntimeException("Token not valid"));
    }
...
//                .andExpect(jsonPath("$.data").doesNotExist());
    ...
    public void recordCommentTest() throws Exception {
        ...
        when(authService.extractUserIdFromToken(token)).thenReturn(userId);
        doNothing().when(commentsService).recordComment(commentDto, userId);
